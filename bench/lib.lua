...
function M.elapsed_ms(ns)
    return math.floor((vim.uv.hrtime() - ns) / 1e6)
end
function M.elapsed_us(ns)
    return (vim.uv.hrtime() - ns) / 1e3
end
function M.now()
    return vim.uv.now()
end
    ...
    if f then
        for line in f:lines() do
            ...
            if pid then
                table.insert(
                    children,
                    { pid = tonumber(pid), rss = tonumber(rss) * 1024, vsz = tonumber(vsz) * 1024, comm = comm }
                )
            end
        end
        f:close()
            ...
            for line in f2:lines() do
                ...
                if pid then
                    local entry = {
                        pid = tonumber(pid),
                        ppid = ppid,
                        rss = tonumber(rss) * 1024,
                        vsz = tonumber(vsz) * 1024,
                        comm = comm,
                        depth = depth,
                    }
            ...
            for pid_line in f:lines() do
                ...
                if pid and not seen[pid] then
                    ...
                    table.insert(procs, {
                        pid = pid,
                        rss = rss_kb * 1024,
                        rss_mb = math.floor(rss_kb / 1024),
                        vsz = vsz_kb * 1024,
                        vsz_mb = math.floor(vsz_kb / 1024),
                        pss = pss,
                        pss_mb = pss and math.floor(pss / 1024 / 1024) or nil,
                        cmd = cmd,
...
-- FORMATTING HELPERS
--=============================================================================
function M.bytes_to_mb(b)
    return string.format("%.1f", b / 1024 / 1024)
end
function M.kb_to_mb(kb)
    return string.format("%.1f", kb / 1024)
end
function M.ms_to_s(ms)
