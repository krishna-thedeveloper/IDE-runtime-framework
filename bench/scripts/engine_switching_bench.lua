...
local function log_state(ctx, s)
    ctx:log(
        string.format(
            "  %s: clients=%d children=%d autocmds=%d modules=%d nvim=%dMB lsp=%dMB total=%dMB cpu=%.1f%%",
            s.label,
            s.n_clients,
            s.n_procs,
            s.autocmds,
            s.modules,
            math.floor(s.nvim_rss / 1024 / 1024),
            math.floor(s.lsp_rss / 1024 / 1024),
            math.floor(s.grand_rss / 1024 / 1024),
            s.cpu
        )
    )
...
function M.run(opts)
    ...
    local s0 = collect_state("before_theme_switching")
    log_state(ctx, s0)
    ...
    for i = 1, math.min(switch_cycles, 200) do
        ...
        if i % 50 == 0 then
            ctx:log(string.format("  Theme switch %d/%d", i, math.min(switch_cycles, 200)))
        end
    end
    ...
    ctx:log(
        string.format(
            "  Theme switching: avg=%.1fms median=%.0fms p95=%.0fms",
            ts_stats.avg,
            ts_stats.median,
            ts_stats.p95
        )
    ...
    local s1 = collect_state("after_theme_switching")
    log_state(ctx, s1)
    ...
    local s2_before = collect_state("before_lsp_attach")
    log_state(ctx, s2_before)
    ...
    if not attached then
        ...
        ctx:record("lifecycle_attach", TS_CLIENT, {
            attached = true,
            attach_ms = attach_ms,
            total_startup_ms = total_attach_ms,
        ...
        ctx:log(
            string.format(
                "  Memory after attach: nvim=%dMB lsp=%dMB total=%dMB",
                math.floor(attach_snap.nvim_rss / 1024 / 1024),
                math.floor(attach_snap.lsp_rss / 1024 / 1024),
        ...
        for _, op in ipairs(ops) do
            ...
            if not ok then
                ctx:log("    ERROR in " .. op.name .. ": " .. tostring(e))
            end
        end
        ...
        local s_after_stop = collect_state("after_stop")
        log_state(ctx, s_after_stop)
        ...
        if attached2 then
            ...
            stop_lsp(TS_CLIENT)
            local s_after_restart = collect_state("after_restart_cleanup")
            log_state(ctx, s_after_restart)
        end
    end
    ...
    for i = 1, cycle_count do
        ...
        if attached_i then
            ...
            table.insert(cycle_memories, {
                cycle = i,
                attach_ms = attach_i_ms,
                nvim_rss = snap_i.nvim_rss,
                lsp_rss = snap_i.lsp_rss,
            })
            stop_lsp(TS_CLIENT)
            vim.wait(500)
            if i % 5 == 0 then
                ctx:log(string.format("  Cycle %d/%d: attach=%dms", i, cycle_count, attach_i_ms))
            end
        end
    end
    ...
    if #cycle_attach_times > 0 then
        ...
        ctx:record("lifecycle_cycling", TS_CLIENT, {
            cycles = #cycle_attach_times,
            avg_ms = ca_stats.avg,
            median_ms = ca_stats.median,
            min_ms = ca_stats.min,
            max_ms = ca_stats.max,
            p95_ms = ca_stats.p95,
        })
        ctx:log(
            string.format(
                "  Cycle attach: avg=%.1fms median=%.0fms min=%.0fms max=%.0fms p95=%.0fms",
                ca_stats.avg,
                ca_stats.median,
                ca_stats.min,
                ca_stats.max,
        ...
        if #cycle_memories >= 3 then
            ...
            ctx:log(
                string.format(
                    "  Growth across %d cycles: nvim=%dKB lsp=%dKB",
                    #cycle_memories,
                    math.floor(nvim_growth / 1024),
                    math.floor(lsp_growth / 1024)
                )
            )
        end
    end
    ...
    local s_before_fc = collect_state("before_file_cycles")
    log_state(ctx, s_before_fc)
    ...
    for i = 1, math.min(switch_cycles, 50) do
        ...
        if i % 10 == 0 then
            ctx:log(string.format("  File cycle %d/%d", i, math.min(switch_cycles, 50)))
        end
    end
    ...
    ctx:log(
        string.format(
            "  File open/close: avg=%.1fms median=%.0fms p95=%.0fms",
            fc_stats.avg,
            fc_stats.median,
            fc_stats.p95
        )
    ...
    local s_after_fc = collect_state("after_file_cycles")
    log_state(ctx, s_after_fc)
    stop_lsp(TS_CLIENT)
    ...
    -- =========================================================================
    ctx:log("\n--- 5. Load & Worst-Case Testing ---")
    local s_before_load = collect_state("before_load_test")
    log_state(ctx, s_before_load)
    ...
    if attached_load then
        ...
        ctx:log(
            string.format(
                "  Load state: nvim=%dMB lsp=%dMB total=%dMB cpu=%.1f%% autocmds=%d",
                math.floor(load_snap.nvim_rss / 1024 / 1024),
                math.floor(load_snap.lsp_rss / 1024 / 1024),
                math.floor(load_snap.grand_rss / 1024 / 1024),
                load_snap.cpu,
                ac_total
            )
        )
        ...
        ctx:record("lifecycle_load_test", "50_files_bulk_open", {
            total_ms = load_elapsed,
            avg_ms = load_stats.avg,
            max_ms = load_stats.max,
        ...
        ctx:log(
            string.format(
                "  Heavy load state: nvim=%dMB lsp=%dMB total=%dMB cpu=%.1f%%",
                math.floor(heavy_snap.nvim_rss / 1024 / 1024),
                math.floor(heavy_snap.lsp_rss / 1024 / 1024),
                math.floor(heavy_snap.grand_rss / 1024 / 1024),
                heavy_snap.cpu
            )
        )
    ...
    local s_after_load = collect_state("after_load_test")
    log_state(ctx, s_after_load)
    stop_lsp(TS_CLIENT)

    local s_after_load_cleanup = collect_state("after_load_cleanup")
    log_state(ctx, s_after_load_cleanup)
    ...
    local env = string.format(
        "ENGINE_NAME=%s PROJ_PATH=%s PROJ_LABEL=%s OUTPUT_FILE=%s",
        "typescript-tools",
        proj_path,
        "cross",
        alt_output
    )
    local alt_cmd = string.format('%s %s --headless -c "luafile %s" -c "qa!"', env, nvim_bin, measure_script)
    os.execute(alt_cmd)
    ...
    if alt_f then
        ...
        if ok then
            if alt_data.attached then
                ...
                ctx:log(
                    string.format(
                        "    memory: nvim=%dMB lsp=%dMB total=%dMB cpu=%.1f%%",
                        alt_data.memory.baseline.nvim_rss_mb,
                        alt_data.memory.baseline.lsp_rss_mb,
                        alt_data.memory.baseline.grand_rss_mb,
                        alt_data.cpu.baseline or 0
                    )
                )
    ...
    ctx:record("lifecycle_orphans", "final_detection", {
        total_children = #all_procs,
        orphans = #final_orphans,
        zombie_count = #final_orphans,
    })
    ...
    if #mem_samples >= 2 then
        ...
        ctx:record("lifecycle_memory", "growth_analysis", {
            initial_rss_kb = first_mem.rss_kb,
            final_rss_kb = last_mem.rss_kb,
            growth_kb = growth_kb,
            growth_percent = math.floor(growth_pct * 100) / 100,
            initial_gc_kb = first_mem.gc_kb,
            final_gc_kb = last_mem.gc_kb,
            samples = #mem_samples,
        })
        ctx:log(
            string.format(
                "  Memory: initial=%dKB final=%dKB growth=%dKB (%.1f%%)",
                first_mem.rss_kb,
                last_mem.rss_kb,
                growth_kb,
                growth_pct
