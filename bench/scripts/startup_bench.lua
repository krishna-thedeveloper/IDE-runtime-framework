...
function M.run(opts)
    ...
    local ctx =
        rm.create_run({ benchmark = "startup", engine = engine, cold = cold_n, warm = warm_n, hot = hot_n }, "startup")
    ...
    local function run_single(label)
        ...
        for _, e in ipairs(entries) do
            if e.ms > total_startuptime then
                total_startuptime = e.ms
            end
    ...
    local function compute_stats(results, label)
        ...
        ctx:log(
            string.format(
                "  avg=%.1f min=%d max=%d median=%d p95=%d p99=%d stddev=%.1f",
                wall_stats.avg,
                wall_stats.min,
                wall_stats.max,
                wall_stats.median,
        ...
        ctx:log(
            string.format(
                "  avg=%.1f min=%.0f max=%.0f median=%.0f p95=%.0f p99=%.0f stddev=%.1f",
                startup_stats.avg,
                startup_stats.min,
                startup_stats.max,
    ...
    for k, v in pairs(contrib) do
        sorted[#sorted + 1] = { name = k, total = v }
    end
