...
function M.run(opts)
  ...
  if pcall(require, "blink.cmp") then engine_name = "blink.cmp" end
  if pcall(require, "cmp") then engine_name = "nvim-cmp" end
  ...
  local function benchmark_completion(buf, label)
    ...
    for i = 1, 10 do
      ...
      local items = result and result.res and result.res.items and #result.res.items
                  or (result and result.res and #result.res) or 0
    ...
    local avg_items = 0
    for _, c in ipairs(item_counts) do avg_items = avg_items + c end
    ...
    ctx:log(string.format("  %s: avg=%.1fms median=%.0fms p95=%.0fms items=%.0f",
      label, stats.avg, stats.median, stats.p95, avg_items))
    return stats
  end
  ...
  ctx:log(string.format("  Memory: baseline=%dKB after=%dKB delta=%dKB",
    mem_baseline, mem_after, mem_after - mem_baseline))
