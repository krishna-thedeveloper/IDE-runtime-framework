...
local function capture(fn, label)
    ...
    if not ok then
        table.insert(results.errors, label .. ": " .. tostring(err))
    end
end
...
for _, op in ipairs(ops) do
    capture(function()
        ...
        vim.lsp.buf_request(buf, op.method, op.params, function(err, res)
            op_result = { err = err, res = res }
            done = true
        end)
        vim.wait(op.timeout or 10000, function()
            return done
        end, 50)
...
for _, c in ipairs(clients) do
    table.insert(
        results.clients,
        { name = c.name, pid = c.rpc and c.rpc.pid, attached_bufs = #(c.attached_buffers or {}) }
    )
end
