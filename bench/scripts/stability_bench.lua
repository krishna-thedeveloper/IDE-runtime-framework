...
function M.run(opts)
  ...
  for i = 1, idle_minutes do
    ...
    for name, count in pairs(client_counts) do
      if count > 1 then
        ...
        ctx:log(string.format("  WARNING: Duplicate LSP client '%s' (%d instances) at minute %d", name, count, i))
      end
    end

    ctx:log(string.format("  minute %2d: nvim=%dMB lsp=%dMB total=%dMB autocmds=%d timers=%d procs=%d cpu=%.1f%%",
      i,
      math.floor(snap.nvim_rss / 1024 / 1024),
      math.floor(snap.lsp_rss / 1024 / 1024),
      math.floor(snap.grand_rss / 1024 / 1024),
      ac_total, timers, #procs, cpu))
  end
  ...
  if #idle_samples >= 2 then
    ...
    ctx:log(string.format("  Idle memory growth: %d MB (%.1f%%) over %d min",
      mem_growth, mem_growth_pct, idle_minutes))
  ...
  for i = 1, 5 do
    ...
    local tbl = {}
    for j = 1, 100000 do tbl[j] = { key = "value_" .. j, data = { 1, 2, 3, 4, 5 } } end
    tbl = nil
  ...
  local avg_freed = 0
  for _, gr in ipairs(gc_results) do avg_freed = avg_freed + gr.freed end
  ...
  if #ts_clients > 0 then
    ...
    ctx:log(string.format("  LSP crash recovery: before=%d after=%d killed=%d",
      clients_before, clients_after, #ts_clients))
  else
  ...
  ctx:log(string.format("  Cleanup: freed %d MB (was %dMB, now %dMB) clients=%d",
