...
function M.run(opts)
    ...
    local function measure_cpu(label, fn)
        ...
        ctx:log(
            string.format(
                "  %s: cpu=%.1f%% lsp_cpu=%.1f%% total=%.1f%% delta=%+.1f%% elapsed=%dms",
                label,
                cpu_after,
                lsp_cpu_after,
                total_cpu_after,
    ...
    measure_cpu("completion", function()
        ...
        local done = false
        vim.lsp.buf_request(ts_buf, "textDocument/completion", params, function()
            done = true
        end)
    ...
    measure_cpu("hover", function()
        ...
        local done = false
        vim.lsp.buf_request(ts_buf, "textDocument/hover", params, function()
            done = true
        end)
    ...
    measure_cpu("definition", function()
        ...
        local done = false
        vim.lsp.buf_request(ts_buf, "textDocument/definition", params, function()
            done = true
        end)
        vim.wait(5000, function()
            return done
    ...
    measure_cpu("references", function()
        ...
        local done = false
        vim.lsp.buf_request(ts_buf, "textDocument/references", params, function()
            done = true
        end)
        vim.wait(5000, function()
            return done
    ...
    measure_cpu("rename", function()
        ...
        local done = false
        vim.lsp.buf_request(ts_buf, "textDocument/rename", params, function()
            done = true
        end)
        vim.wait(5000, function()
            return done
