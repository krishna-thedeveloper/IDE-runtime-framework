...
local function run_single_engine_test(engine_key, proj_path, proj_label)
    ...
    local cmd = string.format('%s %s --headless -c "luafile %s" -c "qa!"', env, nvim_bin, measure_script)
    os.execute(cmd)
    ...
    if not rf then
        return { engine = engine_key, project = proj_label, attached = false, error = "no output file" }
    end
    ...
    if not ok then
        return {
            ...
            project = proj_label,
            attached = false,
            error = "json parse error: " .. tostring(data),
        }
    end
    return data
end
    ...
    for _, proj in ipairs(project_sizes) do
        ...
        for _, engine_key in ipairs(ENGINES) do
            ...
            if r.attached then
                ...
                ctx:log(
                    string.format(
                        "    memory baseline: nvim=%dMB lsp=%dMB total=%dMB",
                        r.memory.baseline.nvim_rss_mb,
                        r.memory.baseline.lsp_rss_mb,
                        r.memory.baseline.grand_rss_mb
                    )
                )
                ...
                ctx:log(
                    string.format(
                        "    memory after ops: nvim=%dMB lsp=%dMB total=%dMB",
                        r.memory.after_ops.nvim_rss_mb,
                        r.memory.after_ops.lsp_rss_mb,
                        r.memory.after_ops.grand_rss_mb
                    )
                )
