...
--- Usage: nvim --headless -c "lua dofile('bench/scripts/dashboard_generator.lua')" -c "qa!"
...
function M.ascii_chart(values, width, height, label)
    ...
    for row = height, 1, -1 do
        ...
        local val_label = ""
        if row == height then
            val_label = string.format(" %.0f", max)
        elseif row == 1 then
            val_label = string.format(" %.0f", min)
        end
...
function M.run()
    local ctx = rm.create_run({ benchmark = "dashboard" }, "dashboard")
        ...
        if run.results then
            ...
            for _, r in ipairs(run.results) do
                if r._category == "startup_stats" and r._name == "cold" then
                    if r.wall_median then
                        table.insert(series.cold_startup_median, r.wall_median)
                    end
                    if r.wall_p95 then
                        table.insert(series.cold_startup_p95, r.wall_p95)
                    end
                end
                if r._category == "lsp_multi" and r._name == "all_servers" then
                    if r.grand_rss_mb then
                        table.insert(series.memory_total, r.grand_rss_mb)
                    end
                end
                if r._category == "lsp_attach" and r._name == "ts_ls" then
                    if r.ms then
                        table.insert(series.lsp_attach, r.ms)
                    end
                end
                if r._category == "completion_latency" and r._name == "ts_ls_completion" then
                    if r.avg_ms then
                        table.insert(series.completion_latency, r.avg_ms)
                    end
                end
                if r._category == "idle_snapshot" then
                    if r.nvim_rss_mb then
                        table.insert(series.nvim_rss, r.nvim_rss_mb)
                    end
                    if r.lsp_rss_mb then
                        table.insert(series.lsp_rss, r.lsp_rss_mb)
                    end
                    if r.clients then
                        table.insert(series.clients, r.clients)
                    end
                    if r.modules then
                        table.insert(series.modules, r.modules)
                    end
                end
            end
        end
    ...
    if #series.nvim_rss > 0 and #series.lsp_rss > 0 then
        ...
        for i = 1, math.min(#series.nvim_rss, #series.lsp_rss) do
            ctx:log(
                string.format(
                    "  Run %d: nvim=%dMB lsp=%dMB total=%dMB",
                    i,
                    series.nvim_rss[i] or 0,
                    series.lsp_rss[i] or 0,
    ...
    add(string.format("**Historical runs:** %d  \n", #historical))
    add("")
    ...
    for name, vals in pairs(series) do
        if #vals > 0 then
            ...
            add(string.format("- **%s**: %d data points, range [%.1f, %.1f]", name, #vals, mn, mx))
        end
    end
    add("")
    add("---")
    ...
    local json_path = ctx.dir .. "/raw/dashboard-data.json"
    ...
    for _, v in pairs(series) do
        if #v > 0 then
            ctx:record("dashboard", "series", { count = #v })
        end
    end
