...
function M.run(opts)
    ...
    local ctx =
        rm.create_run({ benchmark = "plugin_manager", managers = managers, iterations = iterations }, "plugin_manager")
    ...
    for _, mgr in ipairs(managers) do
        ...
        for i = 1, iterations do
            local args = {
                ...
                "--headless",
                "-c",
                string.format("lua require('managers.plugin_manager').setup('%s')", mgr),
                "-c",
                "qa!",
            }
        ...
        ctx:log(
            string.format(
                "  Cold startup: avg=%.1fms median=%.0fms p95=%.0fms (n=%d)",
                startup_stats.avg,
    ...
    table.sort(results, function(a, b)
        return a.startup.avg < b.startup.avg
    end)
