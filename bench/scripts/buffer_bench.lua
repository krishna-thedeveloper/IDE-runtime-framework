...
function M.run(opts)
  ...
  for _, n in ipairs(buffer_counts) do
    ...
    ctx:log(string.format("  %3d buffers: nvim=%dMB lsp=%dMB total=%dMB delta=%dMB switch=%dms",
      n,
  ...
  for _, n in ipairs(window_configs) do
    ...
    elseif n <= 4 then
      vim.cmd("vsplit")
      vim.cmd("split")
      if n == 4 then vim.cmd("vsplit") end
    else
      -- Create grid of splits
      local cols = math.ceil(math.sqrt(n))
      local rows = math.ceil(n / cols)
      for r = 1, rows - 1 do vim.cmd("split") end
      for c = 1, cols - 1 do
        vim.cmd("wincmd j")
        vim.cmd("vsplit")
        for r = 1, rows - 2 do vim.cmd("wincmd k") end
      end
    end
    ...
    ctx:log(string.format("  %2d windows: nvim=%dMB total=%dMB delta=%dMB",
      n,
      math.floor(snap_after.nvim_rss / 1024 / 1024),
      math.floor(snap_after.grand_rss / 1024 / 1024),
  ...
  for _, name in ipairs({ "snacks", "telescope", "fzf-lua" }) do
    ...
    if ok then picker_found = name; break end
  end
