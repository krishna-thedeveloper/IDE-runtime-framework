...
function M.run(opts)
  ...
  if current and current.results and previous and previous.results then
    ...
    for _, r in ipairs(current.results) do
      ...
      if not current_map[key] then current_map[key] = {} end
    ...
    for _, r in ipairs(previous.results) do
      local key = (r._category or "") .. "|" .. (r._name or "")
      if not previous_map[key] then previous_map[key] = {} end
      for k, v in pairs(r) do
        if not k:match("^_") and type(v) == "number" then
      ...
      if prev_metrics then
        for metric, val in pairs(metrics) do
          ...
          if prev_val and prev_val ~= 0 then
            ...
            local best, median = find_best_and_median(
              key:match("^([^|]+)"),
              key:match("|(.+)$"),
              metric
            )
            ...
            elseif pct > 0 then
              entry.severity = "PASS"
              if pct > REGRESSION_THRESHOLDS.slight then entry.severity = "SLIGHT" end
              if pct > REGRESSION_THRESHOLDS.minor then entry.severity = "MINOR" end
              if pct > REGRESSION_THRESHOLDS.warn then entry.severity = "WARN" end
              if pct > REGRESSION_THRESHOLDS.critical then entry.severity = "CRITICAL" end
  ...
  for _, r in ipairs(regressions) do
    if r.severity == "CRITICAL" then
      ...
      ctx:log(string.format("  CRITICAL: %s/%s: %.1f -> %.1f (%+.1f%%)",
        r.key, r.metric, r.previous, r.current, r.delta_pct))
  ...
  for i = 1, math.min(5, #improvements) do
    ...
    ctx:log(string.format("  Improvement: %s/%s: %.1f -> %.1f (%+.1f%%)",
      r.key, r.metric, r.previous, r.current, r.delta_pct))
  end
    ...
    for _, sev in ipairs(severities) do
      ...
      if #sev_items > 0 then
        ...
        for _, r in ipairs(sev_items) do
          add(string.format("| %s/%s | %.2f | %.2f | %+.2f | %+.1f%% |",
            r.key, r.metric, r.previous, r.current, r.delta, r.delta_pct))
        end
        add("")
      end
    end
      ...
      for _, r in ipairs(run.results) do
        ...
        for k, v in pairs(r) do
          if not k:match("^_") and type(v) == "number" then
            ...
            if not metric_history[full_key] then metric_history[full_key] = {} end
  ...
  for full_key, vals in pairs(metric_history) do
    if #vals >= 3 then
      ...
      add(string.format("- **%s**: %s (range: %.2f → %.2f, %.1f%%)",
        full_key, direction, sorted[1], sorted[#sorted], pct_trend))
    end
  end
