...
local function generate_large_file(path, lines, line_content)
  line_content = line_content or "export const x: number = 42; const fn = (a: string, b: number): boolean => { return a.length > b; };"
...
type DeepPartial<T> = {
  ...
  for i = 1, lines do
    fh:write(string.format("export const item_%d: User = { id: %d, name: 'user_%d', email: 'user%d@test.com', role: 'user', metadata: { key: 'value_%d' } };\n", i, i, i, i, i))
  end
  fh:write([=[
...
function M.run(opts)
  ...
  for _, spec in ipairs(file_sizes) do
    ...
    if parse_stats.avg then
      ctx:log(string.format("  parse: avg=%.1fms median=%dms p95=%dms", parse_stats.avg, parse_stats.median, parse_stats.p95))
    else
      ctx:log("  parse: N/A")
    end
    ...
    for i = 1, 5 do
      ...
      if vim.treesitter.highlighter then
        ok = pcall(function() vim.treesitter.highlighter.new(buf) end)
      else
