...
function gen_project(size, config)
  ...
  for i = 1, config.count do
    ...
    for _, imp in ipairs(imports) do f:write(imp .. "\n") end
    f:write("\n")
...
export function handler_%04d_%d(data: Data_%04d): string { return `Processing ${data.name} with value ${data.value}`; }
  ...
  for i = 1, config.count do index:write(string.format("  {} as Data_%04d,\n", i)) end
  index:write("];\n")
  index:close()
  return dir
end
...
function gen_huge()
  ...
  tscfg:write([[{"compilerOptions":{"target":"ESNext","module":"ESNext","strict":true,"outDir":"./dist","rootDir":"./src"},"include":["src/**/*.ts"]}]])
  tscfg:close()
  for batch = 1, 100 do
    for i = (batch-1)*100+1, math.min(batch*100, huge_count) do
      local f = io.open(string.format("%s/src/file_%05d.ts", dir, i), "w")
      f:write(string.format("export interface Data_%05d { id: number; name: string; value: number; active: boolean }\nexport function get_%05d(): Data_%05d { return { id: %d, name: \"item_%d\", value: %d, active: true }; }\n", i, i, i, i, i, i % 100))
      f:close()
    end
    io.write(string.format("\rHuge: %d/%d", math.min(batch*100, huge_count), huge_count)); io.flush()
  end
  io.write("\n")
  local idx = io.open(dir .. "/src/index.ts", "w")
  idx:write("export const all: unknown[] = [\n")
  for i = 1, huge_count do idx:write(string.format("  {} as Data_%05d,\n", i)) end
  idx:write("];\n"); idx:close()
  return dir
end
...
function gen_monorepo()
  ...
  for _, pkg in ipairs(packages) do
    ...
    tsf:write(string.format('{"compilerOptions":{"target":"ESNext","module":"ESNext","strict":true,"outDir":"./dist"},"include":["src/**/*.ts"]}')); tsf:close()
    local deps = {}
    for _, other in ipairs(packages) do if other ~= pkg then table.insert(deps, string.format('"@project/%s": "workspace:*"', other)) end end
    local pjf = io.open(string.format("%s/packages/%s/package.json", dir, pkg), "w")
    pjf:write(string.format('{"name":"@project/%s","dependencies":{%s}}', pkg, table.concat(deps, ","))); pjf:close()
    for i = 1, 50 do
      local f = io.open(string.format("%s/packages/%s/src/module_%03d.ts", dir, pkg, i), "w")
      f:write(string.format("export interface Module_%s_%d { id: string; data: unknown; }\nexport function create_%s_%d(): Module_%s_%d { return { id: \"%s_%d\", data: null }; }\n", pkg, i, pkg, i, pkg, i, pkg, i))
      f:close()
    end
  end
  local rtc = io.open(dir .. "/tsconfig.json", "w")
  rtc:write('{"compilerOptions":{"target":"ESNext","module":"ESNext","strict":true},"references":[\n')
  for _, pkg in ipairs(packages) do rtc:write(string.format('  {"path":"./packages/%s"},\n', pkg)) end
  rtc:write(']}\n'); rtc:close()
  return dir
end
...
export class MegaClass_%d { private items: Map<string, MegaInterface_%d>; private cache: WeakMap<object, string>; constructor(private readonly config: { ttl: number; maxSize: number }) {} async findById(id: number): Promise<MegaInterface_%d | null> { const key = "item_"+id; const cached = this.items.get(key); if (cached) return cached; const result = await this.fetchFromServer(id); if (result) { this.items.set(key, result); this.evictStale(); } return result; } private async fetchFromServer(id: number): Promise<MegaInterface_%d | null> { const response = await fetch('/api/items/'+id); if (!response.ok) return null; return response.json() as Promise<MegaInterface_%d>; } private evictStale(): void { if (this.items.size > this.config.maxSize) { const first = this.items.keys().next().value; if (first) this.items.delete(first); } } *[Symbol.iterator](): IterableIterator<[string, MegaInterface_%d]> { return this.items.entries(); } }
  ...
  for i = 1, 100 do
    mf:write(string.format("function deeplyGenericChain_%d<T,U,V,W,X>(a:T,b:U,c:V,d:W,e:X):Promise<[T,U,V,W,X]>{return Promise.resolve([a,b,c,d,e]);}\nconst result_%d=await deeplyGenericChain_%d(42 as const,\"hello\" as const,true as const,{key:\"value\"} as const,[1,2,3] as const);\n", i, i, i))
  end
  mf:close()

  -- error-bomb.ts (50 deliberate errors)
  local eb = io.open(dir .. "/src/generated/error-bomb.ts", "w")
  eb:write("import { MegaInterface_1 } from './massive-file';\nimport { DeepPartial, DeepRequired } from '../types/complex-generics';\n")
  local errs = {
    'const a: number = "not a number";', 'const b: string = 42;',
    'const c: boolean = "true";', 'const d: string[] = 123;',
  ...
  for i = 1, 50 do eb:write(string.format("// Error %d\n%s\n", i, errs[(i-1)%#errs+1])) end
  eb:close()
  ...
  for i = 1, 10 do
    ...
    if i == 1 then
      cf:write(string.format("import { MegaInterface_1 } from './massive-file';\nexport interface Chain%s extends MegaInterface_1 { chainId: '%s'; chainData: ChainB; }\n", letter, letter))
    elseif i < 10 then
      cf:write(string.format("import { Chain%s } from './chain_%s';\nexport interface Chain%s extends Chain%s { chainId: '%s'; chainData: Chain%s; }\n", string.char(64+i-1), string.char(96+i-1), letter, string.char(64+i-1), letter, next_letter))
    else
      cf:write(string.format("import { Chain%s } from './chain_%s';\nexport interface Chain%s extends Chain%s { chainId: '%s'; chainData: string; }\n", string.char(64+i-1), string.char(96+i-1), letter, string.char(64+i-1), letter))
    end
    cf:close()
  end
  ...
  idx:write("async function demo(): Promise<void> { const item = await mega.findById(1); if (item) { const p: DeepPartial<typeof item> = { name: 'partial' }; console.log(p); } }\n")
  idx:write("type BrandedId = Brand<number, 'UserId'>;\nconst userId: BrandedId = 42 as BrandedId;\ntype TestGeneric = Conditional<string, string, number, boolean>;\n")
  idx:write("demo().catch(console.error);\n")
  idx:close()

  -- tsconfig
  local tsf = io.open(dir .. "/tsconfig.json", "w")
  tsf:write('{"compilerOptions":{"target":"ESNext","module":"ESNext","strict":true,"outDir":"./dist","rootDir":"./src"},"include":["src/**/*.ts"]}'); tsf:close()
...
gen_project("small", sizes.small); print("  small (10 files): done")
gen_project("medium", sizes.medium); print("  medium (100 files): done")
gen_project("large", sizes.large); print("  large (1000 files): done")
gen_huge(); print("  huge (10000 files): done")
gen_monorepo(); print("  monorepo (500 files, 10 packages): done")
gen_worstcase(); print("  worstcase (16 files, 4550-line massive generics + 50 errors): done")
