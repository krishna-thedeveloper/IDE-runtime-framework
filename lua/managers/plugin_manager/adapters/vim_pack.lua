    ...
    return function()
        vim.api.nvim_create_autocmd("VimEnter", {
            ...
            callback = function()
                ...
                if not ok then
                    vim.api.nvim_echo(
                        { { "[vim.pack] Error: " .. tostring(err):sub(1, 200), "WarningMsg" } },
                        false,
                        {}
                    )
                end
...
local function run_config(name, spec_data)
    if spec_data.init then
        ...
        if not ok then
            vim.api.nvim_echo(
                { { "[vim.pack] init error for " .. name .. ": " .. tostring(err):sub(1, 200), "WarningMsg" } },
                false,
                {}
            )
        end
    ...
    if spec_data.config then
        ...
        if not ok then
            vim.api.nvim_echo(
                { { "[vim.pack] config error for " .. name .. ": " .. tostring(err):sub(1, 200), "WarningMsg" } },
                false,
                {}
            )
        end
...
local function run_build(name, build)
    if type(build) == "function" then
        ...
        if not ok then
            vim.api.nvim_echo(
                { { "[vim.pack] build error for " .. name .. ": " .. tostring(err):sub(1, 200), "WarningMsg" } },
                false,
                {}
            )
        end
...
function M.load_plugin(name)
    ...
    if not ok then
        vim.api.nvim_echo(
            { { "[vim.pack] load_plugin(" .. name .. "): " .. tostring(err):sub(1, 200), "WarningMsg" } },
            false,
            {}
        )
    end
