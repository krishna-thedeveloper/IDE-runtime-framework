    ...
    return function()
        vim.api.nvim_create_autocmd("VimEnter", {
            ...
            callback = function()
                ...
                if not ok then
                    vim.api.nvim_echo(
                        { { "[mini.deps] Error: " .. tostring(err):sub(1, 200), "WarningMsg" } },
                        false,
                        {}
                    )
                end
...
local function run_config(name, spec_data)
    if spec_data.init then
        ...
        if not ok then
            vim.api.nvim_echo(
                { { "[mini.deps] init error for " .. name .. ": " .. tostring(err):sub(1, 200), "WarningMsg" } },
                false,
                {}
            )
        end
    ...
    if spec_data.config then
        ...
        if not ok then
            vim.api.nvim_echo(
                { { "[mini.deps] config error for " .. name .. ": " .. tostring(err):sub(1, 200), "WarningMsg" } },
                false,
                {}
            )
        end
...
local function run_build(name, build)
    if type(build) == "function" then
        ...
        if not ok then
            vim.api.nvim_echo(
                { { "[mini.deps] build error for " .. name .. ": " .. tostring(err):sub(1, 200), "WarningMsg" } },
                false,
                {}
            )
        end
...
function M.load_plugin(name)
    if _loaded[name] or _loading[name] then
        return
    end
    _loading[name] = true
    ...
    if not ok then
        vim.api.nvim_echo(
            { { "[mini.deps] load_plugin(" .. name .. "): " .. tostring(err):sub(1, 200), "WarningMsg" } },
            false,
            {}
        )
    end
    ...
    for i, spec in ipairs(specs) do
        ...
        if not vim.uv.fs_stat(install_path) then
            local out = vim.fn.system({
                ...
                "--depth=1",
                "--filter=blob:none",
                "https://github.com/" .. spec.url,
                install_path,
            })
            if vim.v.shell_error ~= 0 then
                vim.api.nvim_echo(
                    { { "[mini.deps] clone failed for " .. spec.url .. ": " .. (out or ""), "ErrorMsg" } },
                    true,
                    {}
                )
            end
